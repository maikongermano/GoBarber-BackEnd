...
let CreateAppointmentService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const appontmentDate = (0, _dateFns.startOfHour)(date); // verifica se a data atual é antes de agora

    if ((0, _dateFns.isBefore)(appontmentDate, Date.now())) {
    ...
    if ((0, _dateFns.getHours)(appontmentDate) < 8 || (0, _dateFns.getHours)(appontmentDate) > 17) {
      throw new _AppError.default('You Can only create appointments between 8am and 5pm');
    }

    const findAppointmenInSameDate = await this.appointmentsRepository.findByDate(appontmentDate, provider_id); // validação de data

    if (findAppointmenInSameDate) {
      throw new _AppError.default('This appoiment is already booked');
    }
    ...
    const dateFormatted = (0, _dateFns.format)(appontmentDate, "dd/MM/yyyy 'às' HH:mm'h'");
    await this.notificationsRepository.create({
      recipient_id: provider_id,
      content: `Novo agendamento para dia ${dateFormatted}`
    }); // invalidado cache

    await this.cacheProvider.invalidade(`provider-appointments:${provider_id}:${(0, _dateFns.format)(appontmentDate, 'yyyy-M-d')}`);
    return appointment;
  }
